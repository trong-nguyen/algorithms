...
SOLUTION:
    * Thinking about transactions as a pair of buy and sell, not individually
    * Thinking dynamically, if I have an optimal solution for the sub problem sized i,
    what does it take to get the solution for the sub problem sized i + 1, utilizing
    the previous result for problem with size i.
    * Math manipulation really makes a ton of difference in performance, in this case
    the factoring out of repetitive components in max operators


    Let's call P[k, i] the maximum profit of the data up to i (i is a non-inclusive bound)
    that utilized k transactions

    Then: the result to P[k, i+1] would be either:
        - not involving S[i] (the new datapoint) which means
            P[k, i+1] = P[k, i]
        - or involving S[i], which means there is a transaction with S[i] the selling point
            P[k, i+1] = max(S[i] - S[j] + P[k-1, j]) for j from [0, i-1] inclusive
            which translates to the optimal solution of subproblem j using k-1 transactions plus one last
            transaction between S[i] and S[j].
        - then P[k, i+1] would be the maximal between those two choices

    For each k from 1 to maximum allowed transactions we have to carry out the above computation

    Naive implementation of the above formula is O(n2k)
    Note that as for the second choice:
        P[k, i+1] = max(S[i] - S[j] + P[k-1, j]) can be rearranged as
            = S[i] + max(-S[j] + P[k-1, j])
        Then the max operator does not involve the new datapoint at i
        In fact the term max(-S[j] + P[k-1, j]) is the product of the price at j and the optimal solution up to j using k-1 transactions
        We call these values (k-1 in total) max_cost and stored in a buffer
        then at each new level i after solving for P[k-1, i]:
            max_cost[k] = max(max_cost[k], -S[i-1] + P[k-1, i])
            and then at i+1:
                P[k, i+1] = S[i] + max_cost[k-1]
            which effectively save one oder of O(n) in execution.

    This solution takes O(nk) time and O(k) space. Since we only need to store:
        - 2 vectors of k sized for P[k, i] and P[k, i-1] in order to compute P[k, i+1]
        - plus the max_cost vector with size k
...
def best_buy(prices, transactions):
    if len(prices) < 2 or transactions < 1:
        return 0
    ...
    mem_2 = [0] * transactions # at j-2
    mem_1 = [max(prices[1] - prices[0], 0)] * transactions # at j-1

    max_cost = [-prices[0]] * transactions # - s_i + P[k-1, i]

    for j in range(2, len(prices)):
        mem = []
        for k in range(transactions):
            if k > 1 and mem[k-1] == mem[k-2]:
                mem += [mem[k-1]] * (transactions - k)
                break
            else:
                profit = mem_1[k]
                max_cost[k] = max(max_cost[k], - prices[j-1] + (mem_2[k-1] if k > 0 else 0))
                mem.append(max(profit, prices[j] + max_cost[k]))

        mem_2, mem_1 = mem_1, mem

    return mem_1[transactions-1]
